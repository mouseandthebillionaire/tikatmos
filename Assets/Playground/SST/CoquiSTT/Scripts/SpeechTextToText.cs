...
public class SpeechTextToText : MonoBehaviour
{
    ...
    protected int _maxRecordingLength = 6;
    ...
    protected float _minimumSpeakingSampleValue = 0.35f;
    ...
    private ISTT _sttClient;

    /// <summary>
    /// Stream used to feed data into the acoustic model.
    /// </summary>
    private STTClient.Models.Stream _sttStream;

    [Header("STT")] public string tfliteFileName;
    public string scorerFileName;
    ...
    {
        string modelPath = Application.dataPath + "/CoquiSTT/models/" + tfliteFileName;
        string externalScorerPath = Application.dataPath + "/CoquiSTT/models/" + scorerFileName;

        _sttClient = new STT(modelPath);
    ...
    private void StartMicrophoneCapture()
    {
        if (_canRecord && _currentlyRecording == false)
        {
            ...
            _audioRecording = Microphone.Start(Microphone.devices[0], true, _maxRecordingLength,_sttClient.GetModelSampleRate());
